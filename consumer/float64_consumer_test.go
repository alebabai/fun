...
var (
	testFloat64ConsumerValue float64
	testFloat64ConsumerError = errors.New("error")
)
...
func TestFloat64Consumer(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			cf: func(t *testing.T) Float64Consumer {
				return func(v float64) error {
					require.Equal(t, testFloat64ConsumerValue, v)
					return nil
				}
			},
		},
		{
			...
			cf: func(t *testing.T) Float64Consumer {
				return func(v float64) error {
					require.Equal(t, testFloat64ConsumerValue, v)
					return testFloat64ConsumerError
				}
			},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			c := tt.cf(t)
			err := c(testFloat64ConsumerValue)
			if err != nil {
				r.EqualError(err, testFloat64ConsumerError.Error())
			} else {
				r.NoError(err)
			}
		})
...
func TestFloat64Consumer_AndThen(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			calls = 0
			err := cc(testFloat64ConsumerValue)
			if err != nil {
				r.EqualError(err, testFloat64ConsumerError.Error())
			} else {
				r.NoError(err)
...
func TestFloat64Consumer_ToMustFloat64Consumer(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.err {
				r.PanicsWithError(testFloat64ConsumerError.Error(), func() {
					mc(testFloat64ConsumerValue)
				})
			} else {
				mc(testFloat64ConsumerValue)
			}
